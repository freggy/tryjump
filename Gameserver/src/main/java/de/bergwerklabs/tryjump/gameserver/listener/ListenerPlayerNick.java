...
/**
 * Created by nexotekHD on 05.05.2016.
 */
public class ListenerPlayerNick implements Listener {
    ...
    {
        if(TryJump.getInstance().getGameSession().getIngame_players().contains(e.getPlayer().getUniqueId()))
        {
            if(e.getType() == NickEvent.NickAction.UNNICK)
            {
