 ...
 * <p>
 * Strategy for randomly selecting {@link LabsSchematic}s. For random selection {@link SecureRandom} is used.
 *
...
public class RandomSelectionStrategies extends UnitSelectionStrategy {
    ...
    private Set<TryJumpUnit> getRandomUnits(@NotNull UnitList list, int amount) {
        ...
        final List<LabsSchematic<TryjumpUnitMetadata>> schematics = new ArrayList<>(list.getDefaultUnits().values());

        for (int i = 0; i < amount; i++) {
            int index = random.nextInt(schematics.size());
            final LabsSchematic<TryjumpUnitMetadata> normalUnit = schematics.get(index);
            final LabsSchematic<TryjumpUnitMetadata> liteUnit = list.getLiteUnits().get(normalUnit.getMetadata().getName());
