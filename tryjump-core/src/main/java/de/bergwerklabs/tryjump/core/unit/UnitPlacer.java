...
public class UnitPlacer {
  ...
  private final SchematicService<TryjumpUnitMetadata> service =
      new SchematicServiceBuilder<TryjumpUnitMetadata>()
          .setDeserializer(new UnitDeserializer())
          .build();

  /**
   * @param easyFolder folder containing subdirectories (../default, ../lite) of units with type
   *     EASY
   * @param mediumFolder folder containing subdirectories (../default, ../lite) of units with type
   *     MEDIUM
   * @param hardFolder folder containing subdirectories (../default, ../lite) of units with type
   *     HARD
   * @param extremeFolder folder containing subdirectories (../default, ../lite) of units with type
   *     EXTREME
   */
  public UnitPlacer(
  ...
  private Map<String, LabsSchematic<TryjumpUnitMetadata>> fromDirectory(@NotNull File dir) {
    return Arrays.stream(Objects.requireNonNull(dir.listFiles()))
        ...
        .collect(
            Collectors.toMap(
                schem -> schem.getMetadata().getName(), Function.identity(), (s1, s2) -> s1));
  }
