...
public class ModuleTypeSummary {
  ...
  private WeightedLootTable<LabsSchematic<TryjumpUnitMetadata>> lootTable =
      new WeightedLootTable<>();

  private File liteFolder;
  private String diffString;

  ModuleTypeSummary(
      @NotNull List<LabsSchematic<TryjumpUnitMetadata>> schematics,
    ...
    this.liteFolder =
        new File(
            TryJumpSession.getInstance().getDataFolder().getAbsolutePath()
                + "/"
                + this.diffString
                + "/lite/");
  ...
  /**
   * Changes the values in the given array so they add up to one. Copy pasta from
   * https://gist.github.com/Alrecenk/7221196 because java.util.Vector doesn't have such a method :(
   *
